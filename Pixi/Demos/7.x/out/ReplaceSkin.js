...
class ReplaceSkin extends BaseDemo {
    constructor() {
        ...
        this._resources.push("resource/you_xin/body/body_ske.json", "resource/you_xin/body/body_tex.json", "resource/you_xin/body/body_tex.png");
        for (let i = 0, l = this._suitConfigs.length; i < l; ++i) {
            for (const partArmatureName of this._suitConfigs[i]) {
                ...
                //
                this._resources.push(dragonBonesJSONPath, textureAtlasJSONPath, textureAtlasPath);
            }
        }
    }
    _onStart() {
        this._factory.parseDragonBonesData(this._pixiResources["resource/you_xin/body/body_ske.json"]);
        this._factory.parseTextureAtlasData(this._pixiResources["resource/you_xin/body/body_tex.json"], this._pixiResources["resource/you_xin/body/body_tex.png"]);
        for (let i = 0, l = this._suitConfigs.length; i < l; ++i) {
            for (const partArmatureName of this._suitConfigs[i]) {
                const path = "resource/you_xin/" + "suit" + (i + 1) + "/" + partArmatureName + "/" + partArmatureName;
                const dragonBonesJSONPath = path + "_ske.json";
                ...
                this._factory.parseTextureAtlasData(this._pixiResources[textureAtlasJSONPath], this._pixiResources[textureAtlasPath]);
            }
        }
        //
